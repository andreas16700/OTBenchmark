...
class Dele: URLSession, URLSessionDelegate{
	
	
	func urlSession(_ session: URLSession, didReceive challenge: URLAuthenticationChallenge) async -> (URLSession.AuthChallengeDisposition, URLCredential?) {
		let cred = URLCredential(trust: challenge.protectionSpace.serverTrust!)
		let disp: URLSession.AuthChallengeDisposition = .useCredential
		return (disp, cred)
	}
...
class Whisk {
	...
	static let session = URLSession(configuration: .default, delegate: sessionDelegate, delegateQueue: nil)
...
//			let session = URLSession(configuration: URLSessionConfiguration.default)
			let (data, _) = try await Self.session.asyncData(with: request)
			do {
